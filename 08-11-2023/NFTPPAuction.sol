...
contract NFTPPAuction is ERC721Holder,Ownable, ReentrancyGuard {
    ...
    function confirmDelivery(address _nftAddress,uint256 _tokenId,address _owner) public{

        address winner = biddingInfo[_nftAddress][_tokenId].higherBidder;
        uint256 winningBid = biddingInfo[_nftAddress][_tokenId].highBid;
        
        require(winner == _msgSender() || owner() == _msgSender(), "Either buyer or Owner can confirm");
        ...
        uint256 remainAmount = winningBid - feeAmount;
         ...
         if (auction.payToken == address(0)) {
            (bool platformFeeTransferSuccess, ) = platformFeeRecipient.call{value: feeAmount}(
                ""
            );
...
//0xB69c331ac1E04108036c2021c75852A2A445cb3b
